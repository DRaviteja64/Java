...
//			    	MonthlyPassMap.put(vehicle1.getvehicleNumber(), slot);
	...
	public void searchVehicle() {
	    ...
	    System.out.print("Enter Vehicle Number : ");
	    String vehicleNo = sc.nextLine().toUpperCase();
	    if (!vehicle.containsKey(vehicleNo)) {
	        System.out.println("Vehicle is not registered.");
	        return;
	    }

	    if (!vehicleMap.containsKey(vehicleNo)) {
	        System.out.println("Vehicle is registered but currently not parked.");
	        return;
	    }
	    ...
	    System.out.printf("| %-10s | %-20s | %-20s | %-20s | %-10s |\n",
	            "Slot", "Owner", "Vehicle", "Type", "Status");
	    System.out.println("+------------+----------------------+----------------------+----------------------+------------+");

	    System.out.printf("| %-10d | %-20s | %-20s | %-20s | %-10s |\n",
	            slot.getSlotNumber(),
	            slot.getPerson().getName(),
	            slot.getVehicle().getvehicleNumber(),
	            slot.getVehicle().getClass().getSimpleName(),
	            "Occupied");
